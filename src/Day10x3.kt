...
class AsyncSolverXXX(machine: Machine) {
    val schems = machine.schematics.sortedByDescending { it.size }
    val checksPrep = schems.map { it.toBitmask() }
    val checks = List(checksPrep.size) { checksPrep.takeLast(checksPrep.size - it).fold(0) { acc, i -> acc or i} }
    var ans = Int.MAX_VALUE / 2
    var cnt = 0

    private val joltage = machine.joltage.toMutableList()
    private var nonNullIndices = (1 shl joltage.size) - 1
    ...
    fun solve(sIdx: Int, score: Int): Int {
        ...
        if (sIdx == schems.size) return ans
        if ((checks[sIdx] and nonNullIndices) == 0) return ans
        if (score + joltage.max() >= ans) return ans

        val mx = schems[sIdx].minOf { joltage[it] }
        var tail = ans

        val prevNonNullIndices = nonNullIndices
        for (i in mx downTo 0) {
            schems[sIdx].forEach {
                joltage[it] -= i
                if (joltage[it] <= 0 && (nonNullIndices and (1 shl it)) != 0)
                    nonNullIndices = nonNullIndices xor (1 shl it)
            }
            tail = min(tail, solve(sIdx + 1, score + i) + i)
            nonNullIndices = prevNonNullIndices
            schems[sIdx].forEach {
                joltage[it] += i
            }
        }
